  ...
  components: {
      ...
      methods: {
        acknowledge(){
            ...
            invoke('create_backup', {number: "6"}).then((res) => {
                ...
                invoke('make_backup', {number: "6"}).then((res) => {
                    ...
                    this.$router.push({ name: 'Setup44' })
                        ...
                        store.commit('setErrorMessage', `Error making backup Error code: Setup43b-1 Response: ${e}`)
                        this.$router.push({ name:'Error' })
                    })
                }).catch((e) => {
                    store.commit('setTest', `error creating and filling backup dir: ${e}`)
                    store.commit('setErrorMessage', `Error creating backup Error code: Setup43b-2 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
                    },
        warn(){
                                ...
                                invoke('packup').then((res) => {
                                    ...
                                    invoke('eject_cd').then((res)=>{
                                        ...
                                        this.loading = false
                                            ...
                                            store.commit('setErrorMessage', `Error ejecting CD Error code: Setup43b-3 Response: ${e}`)
                                            this.$router.push({ name:'Error' })
                                        })   
                                    }).catch((e) => {
                                        store.commit('setTest', `error packing up sensitive info: ${e}`)
                                        store.commit('setErrorMessage', `Error packing up Error code: Setup43b-4 Response: ${e}`)
                                        this.$router.push({ name:'Error' })
                                    })        
                                }).catch((e) => {
                                    store.commit('setTest', `error copying descriptor: ${e}`)
                                    store.commit('setErrorMessage', `Error copying descriptors Error code: Setup43b-5 Response: ${e}`)
                                    this.$router.push({ name:'Error' })
                                })     
                            }).catch((e) => {
                                store.commit('setTest', `error unpacking sensitive info: ${e}`)
                                store.commit('setErrorMessage', `Error unpacking Error code: Setup43b-6 Response: ${e}`)
                                this.$router.push({ name:'Error' })
                            })       
            }).catch((e) => {
                store.commit('setTest', `error reading setup CD: ${e}`)
                store.commit('setErrorMessage', `Error reading setup CD Error code: Setup43b-7 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })
    },
    data(){
