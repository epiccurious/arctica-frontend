    ...
    <div class="form_container">
        <form>
            <div class="checkbox_container">
                ...
                <label for="checkbox">I have inserted CD 5.</label>
            </div>
        </form>
        <div class="btn_container"> 
            <button v-if="this.currentSD == 5 && checkbox" @click="acknowledge()" class="btn">Proceed</Button>
  ...
  components: {
      ...
      methods: {
        acknowledge(){
            ...
            invoke('create_backup', {number: "5"}).then((res) => {
                ...
                invoke('make_backup', {number: "5"}).then((res) => {
                    ...
                    this.$router.push({ name: 'Setup40' })
                        ...
                        store.commit('setErrorMessage', `Error making backup Error code: Setup39b-1 Response: ${e}`)
                        this.$router.push({ name:'Error' })
                    })
                }).catch((e) => {
                    store.commit('setDebug', `error creating and filling backup dir: ${e}`)
                    store.commit('setErrorMessage', `Error creating backup Error code: Setup39b-2 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
                    },
        warn(){
                                ...
                                invoke('packup').then((res) => {
                                    ...
                                    invoke('eject_cd').then((res)=>{
                                        ...
                                        this.loading = false
                                            ...
                                            store.commit('setErrorMessage', `Error ejecting CD Error code: Setup39b-3 Response: ${e}`)
                                            this.$router.push({ name:'Error' })
                                        })   
                                    }).catch((e) => {
                                        store.commit('setDebug', `error packing up sensitive info: ${e}`)
                                        store.commit('setErrorMessage', `Error packing up Error code: Setup39b-4 Response: ${e}`)
                                        this.$router.push({ name:'Error' })
                                    })        
                                }).catch((e) => {
                                    store.commit('setDebug', `error copying descriptor: ${e}`)
                                    store.commit('setErrorMessage', `Error packing up Error code Setup39b-5 Response: ${e}`)
                                    this.$router.push({ name:'Error' })
                                })     
                            }).catch((e) => {
                                store.commit('setDebug', `error unpacking sensitive info: ${e}`)
                                store.commit('setErrorMessage', `Error packing up Error code: Setup39b-6 Response: ${e}`)
                                this.$router.push({ name:'Error' })
                            })       
            }).catch((e) => {
                store.commit('setDebug', `error reading setup CD: ${e}`)
                store.commit('setErrorMessage', `Error reading setup CD Error code: Setup39b-7 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })
    },
    data(){
