            ...
            invoke('create_backup', {number: "7"}).then((res) => {
                ...
                invoke('make_backup', {number: "7"}).then((res) => {
                    ...
                    this.$router.push({ name: 'Setup48' })
                        ...
                        store.commit('setErrorMessage', 'Error making backup Error code Setup47b-1')
                        this.$router.push({ name:'Error' })
                    })
                }).catch((e) => {
                    store.commit('setTest', `error creating and filling backup dir: ${e}`)
                    store.commit('setErrorMessage', 'Error making backup Error code Setup47b-2')
                    this.$router.push({ name:'Error' })
                })
                    },
        warn(){
                                ...
                                invoke('packup').then((res) => {
                                    ...
                                    invoke('eject_cd').then((res)=>{
                                        ...
                                        this.loading = false
                                            ...
                                            store.commit('setErrorMessage', 'Error ejecting CD Error code Setup47b-3')
                                            this.$router.push({ name:'Error' })
                                        })   
                                    }).catch((e) => {
                                        store.commit('setTest', `error packing up sensitive info: ${e}`)
                                        store.commit('setErrorMessage', 'Error packing up Error code Setup47b-4')
                                        this.$router.push({ name:'Error' })
                                    })        
                                }).catch((e) => {
                                    store.commit('setTest', `error copying descriptor: ${e}`)
                                    store.commit('setErrorMessage', 'Error copying descriptors Error code Setup47b-5')
                                    this.$router.push({ name:'Error' })
                                })     
                            }).catch((e) => {
                                store.commit('setTest', `error unpacking sensitive info: ${e}`)
                                store.commit('setErrorMessage', 'Error unpacking Error code Setup47b-6')
                                this.$router.push({ name:'Error' })
                            })       
            }).catch((e) => {
                store.commit('setTest', `error reading setup CD: ${e}`)
                store.commit('setErrorMessage', 'Error reading setup CD Error code Setup47b-7')
                this.$router.push({ name:'Error' })
            })
    },
    data(){
