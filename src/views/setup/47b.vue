                                ...
                                invoke('packup').then((res) => {
                                    ...
                                    invoke('eject_cd').then((res)=>{
                                        ...
                                        this.loading = false
                                            ...
                                            store.commit('setErrorMessage', `Error ejecting CD Error code: Setup47b-3 Response: ${e}`)
                                            this.$router.push({ name:'Error' })
                                        })   
                                    }).catch((e) => {
                                        store.commit('setDebug', `error packing up sensitive info: ${e}`)
                                        store.commit('setErrorMessage', `Error packing up Error code: Setup47b-4 Response: ${e}`)
                                        this.$router.push({ name:'Error' })
                                    })        
                                }).catch((e) => {
                                    store.commit('setDebug', `error copying descriptor: ${e}`)
                                    store.commit('setErrorMessage', `Error copying descriptors Error code: Setup47b-5 Response: ${e}`)
                                    this.$router.push({ name:'Error' })
                                })     
                            }).catch((e) => {
                                store.commit('setDebug', `error unpacking sensitive info: ${e}`)
                                store.commit('setErrorMessage', `Error unpacking Error code: Setup47b-6 Response: ${e}`)
                                this.$router.push({ name:'Error' })
                            })       
            }).catch((e) => {
                store.commit('setDebug', `error reading setup CD: ${e}`)
                store.commit('setErrorMessage', `Error reading setup CD Error code: Setup47b-7 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })
    },
    data(){
