<template>
  ...
  <div v-else class="page">
      <header>
        <h1>Key Generation for Wallet 4 complete</h1>
        <h2>Please power off this machine, remove HW 4, insert HW 5, and power on the machine.</h2> 
      </header> 
      ...
      mounted(){
        ...
        invoke('copy_cd_to_ramdisk').then((res) => {
            ...
            invoke('generate_store_key_pair', {number: this.hwNumber.toString()}).then((res)=>{
                    ...
                    //distribute 2 shards onto HW 4 from setupCD dir
                    invoke('distribute_shards_hw4').then((res)=>{
                            ...
                            invoke('install_hw_deps').then((res) => {
                                            ...
                                            invoke('stop_bitcoind').then((res)=>{
                                                ...
                                                invoke('async_write', {name: 'setupStep', value: this.setupStep}).then((res) => {
                                                    ...
                                                    this.loading = false
                                                        ...
                                                        store.commit('setErrorMessage', `Error with stopping bitcoin daemon Error Code: Setup17-8 Response: ${e}`)
                                    ...
                                    store.commit('setErrorMessage', `Error installing HW dependencies Error code: Setup17-4 Response: ${e}`)
                                    this.$router.push({ name:'Error' })
                                })
                    }).catch((e)=>{
