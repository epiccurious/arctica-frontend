...
<div v-else class="page">
        <header>
        <h1>Hardware Wallet 4</h1>
        <h2>Please remove Hardware Wallet 3 & insert Hardware Wallet 4.</h2>
    </header>    
    <div class="form_container">
        <form>
            <div class="checkbox_container">
                <input type="checkbox" v-model="checkbox" name="checkbox">
                <label for="checkbox">I have inserted HW 4.</label>
            </div>
        </form>
...
export default {
  ...
  methods: {
        acknowledge() {
            this.loading = true
            store.commit('setLoadMessage', 'Creating Hardware Wallet 4...')
            invoke('create_bootable_usb', {number: this.hw, setup: this.setupStep}).then((res) => {
                ...
                store.commit('setDebug', `create bootable HW 4 completed: ${res}`)
                this.loading = false
