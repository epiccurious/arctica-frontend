<template>
    ...
    <div v-else class="page">
        <header>
        <h1>CD 4 Backup</h1>
        <h2>Please remove the Set up CD and insert CD 4.</h2>
    </header> 
  ...
  components: {
      ...
      methods: {
        acknowledge(){
            ...
            invoke('create_backup', {number: "4"}).then((res) => {
                ...
                invoke('make_backup', {number: "4"}).then((res) => {
                    ...
                    this.$router.push({ name: 'Setup36' })
                        ...
                        store.commit('setErrorMessage', `Error making backup Error code: Setup35b-1 Response: ${e}`)
                        this.$router.push({ name:'Error' })
                    })
                }).catch((e) => {
                    store.commit('setDebug', `error creating and filling backup dir: ${e}`)
                    store.commit('setErrorMessage', `Error creating backup Error code: Setup35b-2 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
                    },
        warn(){
                                ...
                                invoke('packup').then((res) => {
                                    ...
                                    invoke('eject_cd').then((res)=>{
                                        ...
                                        this.loading = false
                                            ...
                                            store.commit('setErrorMessage', `Error ejecting CD Error code: Setup35b-3 Response: ${e}`)
                                            this.$router.push({ name:'Error' })
                                        })   
                                    }).catch((e) => {
                                        store.commit('setDebug', `error packing up sensitive info: ${e}`)
                                        store.commit('setErrorMessage', `Error packing up Error code: Setup35b-4 Response: ${e}`)
                                        this.$router.push({ name:'Error' })
                                    })        
                                }).catch((e) => {
                                    store.commit('setDebug', `error copying descriptor: ${e}`)
                                    store.commit('setErrorMessage', `Error copying descriptor Error code: Setup35b-5 Response: ${e}`)
                                    this.$router.push({ name:'Error' })
                                })     
                            }).catch((e) => {
                                store.commit('setDebug', `error unpacking sensitive info: ${e}`)
                                store.commit('setErrorMessage', `Error unpacking Error code: Setup35b-6 Response: ${e}`)
                                this.$router.push({ name:'Error' })
                            })       
            }).catch((e) => {
                store.commit('setDebug', `error reading setup CD: ${e}`)
                store.commit('setErrorMessage', `Error reading setup CD Error code: Setup35b-7 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })
    },
    data(){
