      ...
      components: {
        ...
        invoke('copy_cd_to_ramdisk').then((res) => {
            ...
            invoke('generate_store_key_pair', {number: this.currentSD}).then((res)=>{
                    ...
                    //distribute 1 shard onto sd 6 from setupCD dir
                                        ...
                                        invoke('refresh_setup_cd').then((res)=>{
                                            ...
                                            invoke('packup').then((res)=>{
                                                ...
                                                invoke('async_write', {name: 'setupStep', value: this.setupStep}).then((res) => {
                                                    ...
                                                    store.commit('setErrorMessage', 'Error with async write Error code Setup19b-1')
                                                    this.$router.push({ name:'Error' })
                                                })
                                            }).catch((e) => {
                                                    store.commit('setTest', `packup error: ${e}`)
                                                    store.commit('setErrorMessage', 'Error packing up Error code Setup19b-2')
                                                    this.$router.push({ name:'Error' })
                                                })
                                        }).catch((e)=>{
                                            store.commit('setTest', `refresh setup CD error: ${e}`)
                                            store.commit('setErrorMessage', 'Error refreshing setup CD Error code Setup19b-3')
                                            this.$router.push({ name:'Error' })
                                            })
                                }).catch((e) => {
                                    store.commit('setTest', `install SD deps error: ${e}`)
                                    store.commit('setErrorMessage', 'Error installing SD dependencies Error code Setup19b-4')
                                    this.$router.push({ name:'Error' })
                                })
                    }).catch((e)=>{
                        store.commit('setTest', `distributing 2 shards error: ${e}`)
                        store.commit('setErrorMessage', 'Error distributing 2 shards Error code Setup19b-5')
                        this.$router.push({ name:'Error' })
                    })
            }).catch((e)=>{
                store.commit('setTest', `create wallet error ${e}`)
                store.commit('setErrorMessage', 'Error creating wallet Error code Setup19b-6')
                this.$router.push({ name:'Error' })
            })
        }).catch((e) => {
            store.commit('setTest', `error reading setup CD: ${e}`)
            store.commit('setErrorMessage', 'Error reading setup CD Error code Setup19b-7')
            this.$router.push({ name:'Error' })
        })
        },
        data(){
