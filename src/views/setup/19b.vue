<template>
    ...
    <div v-else class="page">
        <header>
        <h1>Hardware Wallet 6 creation complete</h1>
        <h2>Please power off this machine, remove HW 6, insert HW 7, and power on the machine.</h2>
        </header> 
                            ...
                            invoke('install_sd_deps').then((res) => {
                                    ...
                                    store.commit('setLoadMessage', 'Refreshing setup CD...')
                                        ...
                                        invoke('refresh_cd').then((res)=>{
                                            ...
                                            store.commit('setErrorMessage', `Error refreshing setup CD Error code: Setup19b-3 Response: ${e}`)
                                            this.$router.push({ name:'Error' })
                                            })
                                }).catch((e) => {
                                    store.commit('setDebug', `install SD deps error: ${e}`)
                                    store.commit('setErrorMessage', `Error installing SD dependencies Error code: Setup19b-4 Response: ${e}`)
                                    this.$router.push({ name:'Error' })
                                })
                    }).catch((e)=>{
