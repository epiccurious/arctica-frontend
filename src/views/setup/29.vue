...
<h1>Package 2 is now complete.</h1>
 <h2>Please remove DVD 2 and Hardware Wallet 2.</h2>
 <h2>Place Hardware Wallet 2, and backups CD 2 and DVD 2 into Envelope 2.</h2> 
    ...
    mounted(){
        ...
        invoke('async_write', {name: 'setupStep', value: this.setupStep}).then(() => {
            ...
            .catch((e) => {
                ...
                store.commit('setErrorMessage', `Error with writing async Error code: Setup29-1 Response: ${e}`)
        ...
        invoke('stop_bitcoind').then((res) =>{
            ...
            store.commit('setErrorMessage', `Error with stopping bitcoin daemon Error Code: Setup29-2 Response: ${e}`)
            this.$router.push({ name:'Error' })
        })
    },
}
