<!-- User will be automatically brought to this screen by a text file present on SD 5 which informs arctica to redirect the user to this page (state variable setup2)-->
  ...
  components: {
    ...
    methods: {
        ...
        .catch((e)=> {
          ...
          store.commit('setErrorMessage', `Error reading setup CD Error code: Setup18a-1 Response: ${e}`)
    ...
    mounted(){
        ...
        invoke('mount_internal').then((res)=> {
              ...
              .catch((e)=> {
                ...
                store.commit('setErrorMessage', `Error starting bitcoin daemon Error code: Setup18a-2 Response: ${e}`)
                this.$router.push({ name:'Error' })
              })
          })
        .catch((e)=> {
          store.commit('setTest', `mount internal error: ${e}`)
          store.commit('setErrorMessage', `Error mounting internal disk Error code: Setup18a-3 Response: ${e}`)
          this.$router.push({ name:'Error' })
          })
          }
}
