<template>
    ...
    <div v-else class="page">
        <header>
        <h1>Key Generation for Wallet 5 complete</h1>
        <h2>Please power off this machine, remove HW 5, insert HW 6, and power on the machine.</h2>
        </header> 
...
export default {
    ...
    invoke('copy_cd_to_ramdisk').then((res) => {
        ...
        invoke('generate_store_key_pair', {number: this.hwNumber.toString()}).then((res)=>{
                ...
                invoke('distribute_shards_hw5').then((res)=>{
                                        ...
                                        invoke('stop_bitcoind').then((res)=>{
                                            ...
                                            invoke('async_write', {name: 'setupStep', value: this.setupStep}).then((res) => {
                                                ...
                                                this.loading = false
                                                    ...
                                                    store.commit('setErrorMessage', `Error with stopping bitcoin daemon Error Code: Setup18b-8 Response: ${e}`)
                                                    this.$router.push({ name:'Error' })
                                            })
                                            }).catch((e) => {
                                                store.commit('setDebug', `async write error: ${e}`)
                                                store.commit('setErrorMessage', `Error with async write Error code: Setup18b-1 Response: ${e}`)
                                                this.$router.push({ name:'Error' })
                                            })
                                        }).catch((e) => {
                                                store.commit('setDebug', `packup error: ${e}`)
                                                store.commit('setErrorMessage', `Error packing up Error code: Setup18b-2 Response: ${e}`)
                                                this.$router.push({ name:'Error' })
                                            })
                                    }).catch((e)=>{
                                        store.commit('setDebug', `refresh setup CD error: ${e}`)
                                        store.commit('setErrorMessage', `Error refreshing setup CD Error code: Setup18b-3 Response: ${e}`)
                                        this.$router.push({ name:'Error' })
                                        })
                            }).catch((e) => {
                                store.commit('setDebug', `install HW deps error: ${e}`)
                                store.commit('setErrorMessage', `Error installing HW dependencies Error code: Setup18b-4 Response: ${e}`)
                                this.$router.push({ name:'Error' })
                            })
                }).catch((e)=>{
                    store.commit('setDebug', `distributing 2 shards error: ${e}`)
                    store.commit('setErrorMessage', `Error distributing 2 shards Error code: Setup18b-5 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
        }).catch((e)=>{
            store.commit('setDebug', `create wallet error ${e}`)
            store.commit('setErrorMessage', `Error creating wallet Error code: Setup18b-6 Response: ${e}`)
            this.$router.push({ name:'Error' })
        })
    }).catch((e) => {
        store.commit('setDebug', `error reading setup CD: ${e}`)
        store.commit('setErrorMessage', `Error reading setup CD Error code: Setup18b-7 Response: ${e}`)
        this.$router.push({ name:'Error' })
    })
    },
    data(){
