...
export default {
  ...
  methods: {
        acknowledge() {
            this.loading = true
            store.commit('setLoadMessage', 'Creating SD 5...')
            invoke('create_bootable_usb', {number: this.sd, setup: this.setupStep}).then((res) => {
                ...
                store.commit('setTest', `create bootable SD 5 completed: ${res}`)
                this.loading = false
                this.$router.push({ name:'Setup9' })   
            })
            .catch((e) => {
            store.commit('setTest', `create bootable error: ${e}`)
            store.commit('setErrorMessage', 'Error creating bootable. Error code Setup8-1')
