<template>
    ...
    <div v-else class="page">
        <div class="display_block">
            <h1>Approve Transaction for Signing (3 of 5)</h1>
...
export default {
    ...
    methods: {
        sign(){
            ...
            this.loading = true
            invoke('sign_processed_psbt', {walletname: "delayed", hwnumber: this.currentHW.toString(), progress: "3of5"}).then((res) => {
                store.commit('setDebug', `Signing PSBT: ${res}`)
                store.commit('setLoadMessage', 'Refreshing Transfer CD...')
                invoke('export_psbt', {progress: "3of5"}).then((res) => {
                    store.commit('setDebug', `Exporting PSBT: ${res}`)
                    this.loading=false
                    this.$router.push({name: '3of5success'})
                }).catch((e)=>{
                    store.commit('setDebug', `error exporting PSBT: ${e}`)
                    store.commit('setErrorMessage', `Error exporting PSBT Error Code: sign3of5-4 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
            }).catch((e)=>{
                store.commit('setDebug', `error signing PSBT: ${e}`)
                store.commit('setErrorMessage', `Error Signing PSBT Error Code: sign3of5-3 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })   
        },
    },
    ...
    store.commit('setLoadMessage', 'Unpacking sensitive info...')
        ...
        .catch((e) => {
            ...
            store.commit('setErrorMessage', `Error unpacking sensitive Error code: sign3of5-1 Response: ${e}`)
            this.$router.push({ name:'Error' })
        })
 }
}
