        ...
        mounted(){
            ...
            invoke('stop_bitcoind').then((res)=>{
                ...
                invoke('packup').then((res) =>{
                    ...
                    store.commit('setErrorMessage', `Error with stopping bitcoin daemon Error Code: 4of5success-1 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
            }).catch((e)=>{
                store.commit('setDebug', `error packing up sensitive ${e}`)
                store.commit('setErrorMessage', `Error with packing up sensitive: 4of5success-2 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })
        }, 
        data(){
