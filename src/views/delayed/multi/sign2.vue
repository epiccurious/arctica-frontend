...
export default {
    ...
    methods: {
        sign(){
            ...
            this.loading = true
            invoke('sign_processed_psbt', {walletname: "delayed", hwnumber: this.currentHW.toString(), progress: "2of5"}).then((res) => {
                store.commit('setDebug', `Signing PSBT: ${res}`)
                store.commit('setLoadMessage', 'Refreshing Transfer CD...')
                invoke('export_psbt', {progress: "2of5"}).then((res) => {
                    store.commit('setDebug', `Exporting PSBT: ${res}`)
                    this.loading=false
                    this.$router.push({name: '2of5success'})
                }).catch((e)=>{
                    store.commit('setDebug', `error exporting PSBT: ${e}`)
                    store.commit('setErrorMessage', `Error exporting PSBT Error Code: sign2of5-4 Response: ${e}`)
                    this.$router.push({ name:'Error' })
                })
            }).catch((e)=>{
                store.commit('setDebug', `error signing PSBT: ${e}`)
                store.commit('setErrorMessage', `Error Signing PSBT Error Code: sign2of5-3 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })   
        },
    },
    ...
    store.commit('setLoadMessage', 'Unpacking sensitive info...')
        invoke('unpack').then((res) => {
            ...
            invoke('start_bitcoind', {reindex: false, networkactive: false})
                ...
                store.commit('setErrorMessage', `Error Loading Delayed Wallet Error Code: sign2of5-2 Response: ${e}`)
                this.$router.push({ name:'Error' })
            })
        })     
        .catch((e) => {
            store.commit('setDebug', `error unpacking sensitive: ${e}`)
            store.commit('setErrorMessage', `Error unpacking sensitive Error code: sign2of5-1 Response: ${e}`)
            this.$router.push({ name:'Error' })
        })
 }
}
