...
export default {
    ...
    methods: {
        sign(){
            ...
            //sign the psbt
            invoke('sign_funded_psbt', {walletname: "delayed", hwnumber: this.currentHW.toString(), progress: '2of5'}).then((res) => {
                ...
                store.commit('setErrorMessage', `Error Signing PSBT Error Code: sign2of5-1 Response: ${e}`)
    ...
    mounted(){
        ...
        invoke('decode_funded_psbt', {walletname: "delayed", hwnumber: this.currentHW.toString()}).then((res)=>{
            ...
            this.loading = false
                ...
                store.commit('setErrorMessage', `Error Decoding PSBT Error Code: sign2of5-2 Response: ${e}`)
                this.$router.push({ name:'Error' })
        })
 }
}
